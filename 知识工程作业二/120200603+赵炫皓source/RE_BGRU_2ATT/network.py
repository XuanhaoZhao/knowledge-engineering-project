...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        gru_cell_forward = tf.keras.layers.GRUCell(gru_size)
        gru_cell_backward = tf.keras.layers.GRUCell(gru_size)

        if is_training and settings.keep_prob < 1:
            gru_cell_forward = tf.compat.v1.nn.rnn_cell.DropoutWrapper(gru_cell_forward, output_keep_prob=settings.keep_prob)
            gru_cell_backward = tf.compat.v1.nn.rnn_cell.DropoutWrapper(gru_cell_backward, output_keep_prob=settings.keep_prob)

        cell_forward = tf.compat.v1.nn.rnn_cell.MultiRNNCell([gru_cell_forward] * settings.num_layers)
        cell_backward = tf.compat.v1.nn.rnn_cell.MultiRNNCell([gru_cell_backward] * settings.num_layers)
        ...
        self._initial_state_forward = cell_forward.get_inital_state(total_num, tf.float32)
        self._initial_state_backward = cell_backward.get_inital_state(total_num, tf.float32)
        ...
        tf.summary.scalar('loss', self.total_loss)
        # regularization
        self.l2_loss = tf.keras.regularizers.Regularizer(regularizer=tf.contrib.layers.l2_regularizer(0.0001),
